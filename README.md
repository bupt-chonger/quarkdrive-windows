...
配置文件保存在当前用户 `%APPDATA%\QuarkDrive\config.json`。程序只使用夸克网盘网页端会话，不会通过第三方服务交换登录令牌。
...
2. 接入分片上传、重命名和移动 API；
3. SQLite 持久化传输队列、远端版本、冲突副本与崩溃恢复；
4. 远端变更轮询、缓存淘汰、断网重试、限流与端到端集成测试。

## 安全边界

- `unregister` 只注销 CFAPI 同步根，不删除本地内容；
- 当前未实现上传、重命名和移动回调；删除回调会先提交夸克网盘删除请求，远端请求失败时本地删除会被拒绝；
- 在完成双向同步和冲突处理前，请不要把挂载目录作为唯一文件副本。
